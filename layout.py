...
def layoutGrid(nrows, ncols, row_widths, col_heights, hspace, wspace, bottom, top, left, right):
	'''
		...
		args:
			...
			hspace: float
				Horizontal space between cells in mm.
			wspace: float
				Vertical space between cells in mm.
			bottom: float
	...
	overall_width = sum(row_widths)+(nrows-1)*wspace+left+right
	overall_height = sum(col_heights)+(ncols-1)*hspace+bottom+top

	# Declare figreu width overall extensions in inches
	fig = plt.figure(figsize = (overall_width/25.4, overall_height/25.4))

	# Define fractions left, right, bottom and top
	...
	wspace_frac = wspace/average_row_widths
	hspace_frac = hspace/average_col_heights
